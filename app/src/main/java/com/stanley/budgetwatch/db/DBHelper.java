...
public class DBHelper extends SQLiteOpenHelper {
    ...
    /**
     * ALL strings used in the budget table
     * */
    public static class BudgetDbIds{
    ...
    /*
    * Send a notification that the transaction table has changed
    * */
    private void sendChangeNotification() {
        context.sendBroadcast(new Intent(TransactionDatabaseChangedReceiver.ACTION_DATABASE_CHANGED));
    }
     ...
     * false otherwise
     */
    public boolean insertBudget(SQLiteDatabase db, final String name, final int max){
    ...
    public boolean updateBudget(final String name, final int max){
        ...
        SQLiteDatabase db = getWritableDatabase();
        int rowsUpdated = db.update(BudgetDbIds.TABLE, contentValues, BudgetDbIds.NAME + "=?", new String[]{name});
        db.close();
    ...
    public boolean deleteBudget(final String name){
        SQLiteDatabase db = getWritableDatabase();
        int rowsDeleted = db.delete(BudgetDbIds.TABLE, BudgetDbIds.NAME + "=?", new String[]{name});
        db.close();
    ...
    public Budget getBudgetStoredOnly(final  String name){
        ...
        Cursor cursor = db.rawQuery("select * from " + BudgetDbIds.TABLE + " where " + BudgetDbIds.NAME + "=?", new String[]{name});

        Budget budget = null;

        if(cursor.getCount() == 1){
            cursor.moveToNext();
            String goalName = cursor.getString(cursor.getColumnIndexOrThrow(BudgetDbIds.NAME));
            int goalMax = cursor.getInt(cursor.getColumnIndexOrThrow(BudgetDbIds.MAX));
    ...
    public List<Budget> getBudgets(long startDateMs, long endDateMs){
        ...
        final int MONT_PER_YEAR = 12;
        int startMonths = calendar.get(Calendar.YEAR) * MONT_PER_YEAR + calendar.get(Calendar.MONTH);
        calendar.setTimeInMillis(endDateMs);
        int endMonths = calendar.get(Calendar.YEAR) * MONT_PER_YEAR + calendar.get(Calendar.MONTH);
     ...
     */
    public List<String>  getBudgetName(){
        SQLiteDatabase db = getReadableDatabase();
        Cursor data = db.rawQuery("select " + BudgetDbIds.NAME + " from " + BudgetDbIds.TABLE + " ORDER BY "
        + BudgetDbIds.NAME , null);

        LinkedList<String> budgetName = new LinkedList<>();
